...
public class SpoilerEffect2 {
    ...
    public void draw(Canvas canvas, View view, int w, int h, float alpha, boolean toBitmap) {
        ...
        // ФИКС "спойлер не доходит до низа в ленте (но норм в обычном чате)":
        // поворот/отражение "для разнообразия" ниже пивотился вокруг центра
        // СТАРОГО фиксированного размера текстуры (ow/2, oh/2) — это совпадает с
        // центром реального контента ТОЛЬКО пока контент меньше или равен этому
        // фиксированному размеру (обычный чат-баббл). В ленте карточка часто
        // ВЫШЕ (см. ветку scale ниже) — после растяжения реальный центр
        // контента это (w/2, h/2), а НЕ (ow/2, oh/2). Поворот/отражение вокруг
        // неправильной точки сдвигает уже растянутый прямоугольник в сторону —
        // именно поэтому 3 из 4 "вариантов разнообразия" (с поворотом/
        // отражением) обрезали низ, а 4-й (без трансформации) выглядел нормально
        // случайно. Пивот исправлен на центр РЕАЛЬНОГО размера вьюхи (w/2, h/2).
        if ((index % 4) == 1) {
        ...
        // ФИКС "спойлер не доходит до низа у высоких карточек в ленте": раньше
        // масштабирование ("if (w > ow || h > oh)") шло с точкой опоры в центре
        // СТАРОГО (маленького, ow x oh — фиксированный размер общего GL-инстанса
        // из getSize(), 720-1280px в зависимости от performance class) размера.
        // При растяжении вверх/вниз от центра старого маленького бокса итоговая
        // отрисованная область не совпадала с реальным View (0,0)-(w,h): низ
        // оставался не закрыт. В оригинальных чат-бабблах Telegram эта ветка почти
        // никогда не срабатывала (там медиа мельче общего sz), а у нас карусель
        // ленты может быть выше (до 560dp) — отсюда и баг именно на длинных постах.
