...
public class PotokDebugLog {
    ...
    // ⚠️ По прямому требованию пользователя переключено на 974 (t.me/komissariatforsvoix/974):
    // ВЕСЬ путь отрисовки/бинда/скролла закрытого превью round video для ЭТОГО
    // поста логируется безусловно (тег TARGETPOST), без фильтра по гипотезам —
    // см. дополнительные точки логирования, добавленные в ImageReceiver.java
    // (roundRect ДО асимметричной ветки, сама ветка, финальный canvas.draw*).
    public static volatile long TARGET_MESSAGE_ID = 974L;
